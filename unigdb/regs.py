...
class Architecture(object):
    """Generic metaclass for the architecture supported by unigdb."""
    ...
    def get_ra(self, insn, frame):
        ...
        if self.is_ret(insn):
            ...
            if insn.mnemonic == "pop":
                ra_addr = unigdb.arch.CURRENT_ARCH.sp + (len(insn.operands) - 1) * unigdb.arch.ptrsize
                ra = lazy_dereference(ra_addr)
            elif insn.mnemonic == "ldr":
                return lazy_dereference(unigdb.arch.CURRENT_ARCH.sp)
...
class X86(Architecture):
    ...
    def get_ra(self, insn, frame):
        ra = None
        if self.is_ret(insn):
            ra = lazy_dereference(unigdb.arch.CURRENT_ARCH.sp)
        if frame.older():
            ra = frame.older().pc()
        return ra
    ...
    def get_ith_parameter(self, i):
        sp = unigdb.arch.CURRENT_ARCH.sp
        sz = unigdb.arch.CURRENT_ARCH.ptrsize
        loc = sp + (i * sz)
        val = unigdb.memory.u32(loc)
